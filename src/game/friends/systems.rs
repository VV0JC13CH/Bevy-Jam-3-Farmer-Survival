...
pub fn mouse_spawn(
    ...
    if spawn_mouse {
        ...
        let animation_indices_idle = AnimationIndicesIdle {
            first: MAX_SPECIES_OF_FRIENDS + 0,
            second: MAX_SPECIES_OF_FRIENDS * 2 + 0,
        };
        let animation_indices_running = AnimationIndicesRunning {
            first: MAX_SPECIES_OF_FRIENDS * 3 + 0,
            second: MAX_SPECIES_OF_FRIENDS * 4 + 0,
        };
...
pub fn friends_movement(
    mut commands: Commands,
    mut friend_query: Query<(&mut Friend, &Transform), With<Friend>>,
    camera_query: Query<Entity, With<Camera>>,
) {
...
pub fn friends_animate(
    ...
    for (transform, indices_idle, indices_running, mut timer, mut sprite, mut friend) in &mut query
    {
